...
The system provides a full-stack solution for autonomous visual docking. By utilizing a wrist-mounted camera, the robot identifies environmental markers and adjusts its end-effector pose in real-time.
...
5.  **Control Loop:** The **Movement Controller** calculates the delta between the current tool pose and the target tag pose, sending incremental updates via the Kinova Kortex API.
...
* Provides visual debugging overlays (coordinate axes, distance markers).

### 🦾 Robot Interface
* Leverages the **Kinova Kortex API**.
* Synchronizes real-time feedback of the robot's current Cartesian pose.
* Manages safety-rated motion commands.

### ⚙️ Movement Controller
* Implements incremental alignment logic.
...
└── scripts/                    # Entry points and main control
